...
public enum VioCampaignPartnerAPI {
    ...
    /// POST `/v2/mobile/campaigns/:campaignId/register-device` — registers APNs/FCM token for offline push (when supported by API).
    public static func registerDevice(
        ...
        print("🎯 [VioCampaignPartnerAPI] register-device ← HTTP \(http.statusCode) body: \(preview)")
    }
}
