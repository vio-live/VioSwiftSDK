...
public struct TimelineDataGenerator {
    ...
    public static func generateBarcelonaPSGTimeline() -> [AnyTimelineEvent] {
        ...
        // Tweet (uses socialAccounts[0] from config)
        ...
        // Casting Product (from config, URLs from productMappings)
        if products.count > 0 {
            let p = products[0]
    ...
    public static func generateRealMadridBarcelonaTimeline() -> [AnyTimelineEvent] {
        var events: [AnyTimelineEvent] = []
